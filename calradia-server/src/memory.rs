...
impl Memory {
    ...
    pub fn find(&self, campaign: u32, job: u32) -> Result<Option<Existing>, MemoryFailure> {
        self.with(|c| {
            Ok(c.query_row(
                "SELECT fingerprint, npc_text FROM turns WHERE campaign_id = ?1 AND job = ?2",
                params![campaign, job],
                |r| {
                    Ok(Existing {
                        fingerprint: r.get(0)?,
    ...
    pub fn history(
        ...
        self.with(|c| {
            ...
            let rows = stmt.query_map(params![campaign, head, MAX_CHAIN, character], |r| {
                ...
                Ok((turn, r.get::<_, bool>(6)?, r.get::<_, u32>(7)?))
            })?;
            for row in rows {
                let (turn, mine, chain_len) = row?;
