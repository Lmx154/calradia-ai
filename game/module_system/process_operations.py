...
def load_quick_strings(export_dir):
  quick_strings = []
  try:
    file = open(export_dir + "quick_strings.txt", "r", encoding="cp1254")
...
def save_quick_strings(export_dir, quick_strings):
  file = open(export_dir + "quick_strings.txt", "w", encoding="cp1254")
  file.write("%d\n"%len(quick_strings))
...
def load_variables(export_dir,variable_uses):
  variables = []
  try:
    file = open(export_dir + "variables.txt","r", encoding="cp1254")
  ...
  try:
    file = open(export_dir + "variable_uses.txt","r", encoding="cp1254")
    var_list = file.readlines()
    file.close()
    for v in  var_list:
      vv = v.strip()
...
def save_variables(export_dir,variables_list,variable_uses):
  file = open(export_dir + "variables.txt","w", encoding="cp1254")
  for i in range(len(variables_list)):
    file.write("%s\n"%variables_list[i])
  file.close()
  file = open(export_dir + "variable_uses.txt","w", encoding="cp1254")
  for i in range(len(variables_list)):
    file.write("%d\n"%variable_uses[i])
  file.close()
...
def load_tag_uses(export_dir):
  ...
  try:
    file = open(export_dir + "tag_uses.txt","r", encoding="cp1254")
    var_list = file.readlines()
    file.close()
    for v in  var_list:
      vv = v.strip().split(';')
...
def save_tag_uses(export_dir,tag_uses):
  file = open(export_dir + "tag_uses.txt","w", encoding="cp1254")
