...
#           (try_end),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (24,
   []),
  # --- Calradia AI (milestones 4-6): forward world nodes to calradia-server ---
  # Every frame on the world map: if the transport is idle, send one Native log entry, the
  # daily snapshot or the daily tick (script_cai_background_send; docs/protocol-v1.md).
  (0,
   [(call_script, "script_cai_background_send"),
    ]),
  # --- end Calradia AI ---
]
