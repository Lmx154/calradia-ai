...
from module_scripts import (CAI_OP_TALK, CAI_OP_RESULT, CAI_OP_CANCEL, CAI_OP_ACK,
...
s68 = 68  # name retained while the conversation presentation is open
# --- end Calradia AI ---
...
##        (gt, ":total_wage", 0),
  ...
  # --- Calradia AI (milestone 2): talk with {s68}; see docs/protocol-v1.md ---
  ...
  ("cai_talk", prsntf_manual_end_only, mesh_load_window, [
    (ti_on_presentation_load,
     [(set_fixed_point_multiplier, 1000),
      (val_add, "$cai_prsnt_inst", 1),
      # Presentation time starts at 0 on load. A request still outstanding from an earlier
      # instance ($cai_tx_inst != $cai_prsnt_inst) is aged from this moment.
      (assign, "$cai_now_ms", 0),
      ...
      (str_clear, s66),
      (assign, "$cai_ui_open", 1),
      (assign, "$cai_ack_owed_job", 0),
      (assign, "$cai_ack_confirmed", 0),
      (assign, "$cai_memory_confirmed", 1),
      (str_store_string, s68, "@Hrodvar"),
      (try_begin),
        (gt, "$cai_npc", 0),
        (str_store_troop_name, s68, "$cai_npc"),
        (assign, "$cai_memory_confirmed", 0),
        (call_script, "script_cai_new_id", 0, "$cai_session_a"),
        (assign, "$cai_session_a", reg0),
        (call_script, "script_cai_new_id", 0, "$cai_session_b"),
        (assign, "$cai_session_b", reg0),
      (try_end),

      (create_text_overlay, reg1, "@{s68}", tf_center_justify),
      (position_set_x, pos1, 500),
      (position_set_y, pos1, 680),
      (overlay_set_position, reg1, pos1),
      (position_set_x, pos1, 1500),
      ...
      (create_text_overlay, "$cai_obj_status", "@Idle", tf_left_align),
      (position_set_x, pos1, 80),
      (position_set_y, pos1, 220),
      (overlay_set_position, "$cai_obj_status", pos1),
      (create_text_overlay, "$cai_obj_warn", "str_empty_string", tf_left_align),
      (overlay_set_color, "$cai_obj_warn", 0xCC0000),
      (position_set_x, pos1, 80),
      (position_set_y, pos1, 190),
      ...
      (position_set_x, pos1, 80),
      (position_set_y, pos1, 130),
      (overlay_set_position, "$cai_obj_text_box", pos1),

      (create_button_overlay, "$cai_obj_say", "@Say", tf_center_justify),
      (position_set_x, pos1, 150),
      (position_set_y, pos1, 60),
      (overlay_set_position, "$cai_obj_say", pos1),
      (create_button_overlay, "$cai_obj_cancel", "@Cancel", tf_center_justify),
      (position_set_x, pos1, 330),
      (position_set_y, pos1, 60),
      (overlay_set_position, "$cai_obj_cancel", pos1),
      (create_button_overlay, "$cai_obj_reset", "@Reset (restart calradia-server first)", tf_center_justify),
      (position_set_x, pos1, 600),
      (position_set_y, pos1, 60),
      (overlay_set_position, "$cai_obj_reset", pos1),
      (overlay_set_display, "$cai_obj_reset", 0),
      (create_button_overlay, "$cai_obj_close", "@Close", tf_center_justify),
      (position_set_x, pos1, 870),
      (position_set_y, pos1, 60),
      (overlay_set_position, "$cai_obj_close", pos1),

      (create_button_overlay, "$cai_obj_continue", "@Continue this history (latest save)", tf_center_justify),
      (position_set_x, pos1, 300),
      (position_set_y, pos1, 100),
      (overlay_set_position, "$cai_obj_continue", pos1),
      (create_button_overlay, "$cai_obj_branch", "@New empty memory branch", tf_center_justify),
      (position_set_x, pos1, 720),
      (overlay_set_position, "$cai_obj_branch", pos1),
      (try_begin),
        (eq, "$cai_memory_confirmed", 0),
        (assign, reg(70), "$cai_campaign_a"),
        (assign, reg(71), "$cai_campaign_b"),
        (assign, reg(72), "$cai_branch_a"),
        (assign, reg(73), "$cai_branch_b"),
        (overlay_set_text, "$cai_obj_reply", "@Choose memory history before speaking.^^Campaign: {reg70}-{reg71}^Branch: {reg72}-{reg73}^^Continue only if this save follows the latest conversations in this branch. If you loaded an older save, or are unsure, choose a new empty branch.^^Save the game after choosing a branch. Memories are stored separately from Warband saves. A new branch starts with no conversations."),
        (overlay_set_text, "$cai_obj_status", "@Memory history needs your choice."),
        (try_begin),
          (eq, "$cai_campaign_a", 0),
          (overlay_set_text, "$cai_obj_continue", "@Start campaign memory"),
        (try_end),
      (else_try),
        (overlay_set_display, "$cai_obj_continue", 0),
        (overlay_set_display, "$cai_obj_branch", 0),
      (try_end),
      (presentation_set_duration, 999999),
      ]),
    ...
    (ti_on_presentation_run,
     [(store_trigger_param_1, ":now"), # ms since load
      ...
      (try_begin),
        ...
        (call_script, "script_cai_close"),
      (try_end),

      (eq, "$cai_ui_open", 1),
      (try_begin),
        (eq, "$cai_ack_confirmed", 1),
        (assign, "$cai_ack_confirmed", 0),
        (overlay_set_text, "$cai_obj_status", "@Reply displayed and remembered."),
      (try_end),

      # A reply or a terminal answer from the callback.
      (try_begin),
        (eq, "$cai_reply_new", 1),
        (assign, "$cai_reply_new", 0),
        ...
        (try_begin),
          ...
          (str_store_string, s60, "@You: {s64}^{s68}: {s67}"),
          (overlay_set_text, "$cai_obj_status", "@{s68} has answered."),
        (else_try),
        ...
        (try_begin),
          (gt, "$cai_npc", 0),
          (eq, "$cai_conv_state", CAI_CONV_READY),
          (assign, "$cai_ack_owed_job", "$cai_conv_job"),
          (overlay_set_text, "$cai_obj_status", "@Reply displayed; waiting for memory confirmation. Keep this window open."),
        (try_end),
      (try_end),
      ...
      (try_begin),
        ...
        (str_store_string, s67, "@{s68} does not answer."),
      ...
      (try_begin),
        (eq, "$cai_tx_rid", 0),
        (try_begin),
          (gt, "$cai_cancel_owed_job", 0),
          (call_script, "script_cai_tx_send", CAI_OP_CANCEL, "$cai_cancel_owed_job"),
          (assign, "$cai_cancel_owed_job", 0),
        (else_try),
          (gt, "$cai_ack_owed_job", 0),
          (store_sub, ":since_ack", ":now", "$cai_tx_sent_ms"),
          (ge, ":since_ack", CAI_POLL_MS),
          (call_script, "script_cai_tx_send", CAI_OP_ACK, "$cai_ack_owed_job"),
        (else_try),
          (eq, "$cai_conv_state", CAI_CONV_PENDING),
          (store_sub, ":since_send", ":now", "$cai_tx_sent_ms"),
          (ge, ":since_send", CAI_POLL_MS),
          (call_script, "script_cai_tx_send", CAI_OP_RESULT, "$cai_conv_job"),
        (try_end),
      (try_end),

      # Stuck request. 0 = none, 1 = ours unanswered for 5 s, 2 = one from an earlier
      # instance, 3 = the same, 15 s after load (Reset offered).
      (assign, ":warn", 0),
      (try_begin),
        (neq, "$cai_tx_rid", 0),
        (try_begin),
          (eq, "$cai_tx_inst", "$cai_prsnt_inst"),
          (store_sub, ":age", ":now", "$cai_tx_sent_ms"),
          (ge, ":age", CAI_STUCK_MS),
          (assign, ":warn", 1),
        (else_try),
      ...
      (try_end),
      (try_begin),
        (neq, ":warn", "$cai_ui_warn"),
        (assign, "$cai_ui_warn", ":warn"),
        (try_begin),
          (eq, ":warn", 1),
          (overlay_set_text, "$cai_obj_warn", "@calradia-server is not answering; restart it."),
        (else_try),
          (eq, ":warn", 2),
          (overlay_set_text, "$cai_obj_warn", "@Waiting for an earlier request to calradia-server..."),
        (else_try),
          (eq, ":warn", 3),
          (overlay_set_text, "$cai_obj_warn", "@An earlier request to calradia-server is stuck."),
        (else_try),
          (overlay_set_text, "$cai_obj_warn", "str_empty_string"),
        (try_end),
        (try_begin),
          (eq, ":warn", 3),
          (overlay_set_display, "$cai_obj_reset", 1),
        (else_try),
          (overlay_set_display, "$cai_obj_reset", 0),
        (try_end),
      (try_end),
      ...
      (try_begin),
        ...
        (neg|is_between, "$cai_conv_state", CAI_CONV_SUBMITTING, CAI_CONV_PENDING + 1),
          (eq, "$cai_memory_confirmed", 1),
          (eq, "$cai_ack_owed_job", 0),
        (neg|str_is_empty, s66),
        (overlay_set_alpha, "$cai_obj_say", 0xFF),
      (else_try),
        (overlay_set_alpha, "$cai_obj_say", 0x60),
      (try_end),
      (try_begin),
        (is_between, "$cai_conv_state", CAI_CONV_SUBMITTING, CAI_CONV_PENDING + 1),
        (overlay_set_alpha, "$cai_obj_cancel", 0xFF),
      (else_try),
        (overlay_set_alpha, "$cai_obj_cancel", 0x60),
      (try_end),
      ]),
    ...
    (ti_on_presentation_event_state_change,
     [(store_trigger_param_1, ":object"),
      (try_begin),
        (this_or_next|eq, ":object", "$cai_obj_continue"),
        (eq, ":object", "$cai_obj_branch"),
        (eq, "$cai_memory_confirmed", 0),
        (try_begin),
          (eq, "$cai_campaign_a", 0),
          (call_script, "script_cai_new_id", "$cai_now_ms", 0),
          (assign, "$cai_campaign_a", reg0),
          (call_script, "script_cai_new_id", "$cai_now_ms", 0),
          (assign, "$cai_campaign_b", reg0),
        (try_end),
        (try_begin),
          (this_or_next|eq, ":object", "$cai_obj_branch"),
          (eq, "$cai_branch_a", 0),
          (call_script, "script_cai_new_id", "$cai_now_ms", "$cai_branch_a"),
          (assign, "$cai_branch_a", reg0),
          (call_script, "script_cai_new_id", "$cai_now_ms", "$cai_branch_b"),
          (assign, "$cai_branch_b", reg0),
        (try_end),
        (assign, "$cai_memory_confirmed", 1),
        (overlay_set_display, "$cai_obj_continue", 0),
        (overlay_set_display, "$cai_obj_branch", 0),
        (overlay_set_text, "$cai_obj_reply", "str_empty_string"),
        (overlay_set_text, "$cai_obj_status", "@Ready. Save your game after choosing a new memory branch."),
      (else_try),
      ...
      (else_try),
        ...
        (try_begin),
          ...
          (overlay_set_text, "$cai_obj_reply", "@You: {s64}^{s68} is thinking...^^{s60}^^{s61}^^{s62}"),
          (overlay_set_text, "$cai_obj_status", "@Waiting for {s68}..."),
          (call_script, "script_cai_tx_send", CAI_OP_TALK, "$cai_conv_job"),
          (str_clear, s66),
          (overlay_set_text, "$cai_obj_text_box", "str_empty_string"),
        (try_end),
      (else_try),
        (eq, ":object", "$cai_obj_cancel"),
        # Logical cancel: the transport is left alone; /v1/cancel goes out once it is idle.
        (try_begin),
          (is_between, "$cai_conv_state", CAI_CONV_SUBMITTING, CAI_CONV_PENDING + 1),
          (assign, "$cai_conv_state", CAI_CONV_CANCELED),
          (assign, "$cai_cancel_owed_job", "$cai_conv_job"),
          (str_store_string, s67, "@You stopped waiting for {s68}."),
          (assign, "$cai_reply_new", 1),
        (try_end),
      (else_try),
        (eq, ":object", "$cai_obj_reset"),
        # Only for a request older than this presentation, 15 s after load.
        (try_begin),
          (neq, "$cai_tx_rid", 0),
          (neq, "$cai_tx_inst", "$cai_prsnt_inst"),
